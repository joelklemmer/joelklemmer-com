...
- **Steps in order:** clear `.next`, format:check, pnpm audit (critical), lint, security-validate, then all \*-validate targets below, test, build, restore next-env.d.ts, a11y (with SKIP_A11Y_BUILD=1)

## CI jobs (evidence from .github/workflows/ci.yml)

| Job         | What runs                                                 | Failure if                                            |
| ----------- | --------------------------------------------------------- | ----------------------------------------------------- |
| verify-fast | format:check, lint, all validators below, web:test        | Any step exits non-zero                               |
| build       | nx run web:build, restore next-env.d.ts, git status clean | Build fails or uncommitted changes                    |
| a11y        | nx run web:a11y (Playwright + axe)                        | A11y violations or script failure                     |
