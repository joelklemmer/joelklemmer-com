...
const files = ['en.report.json', 'en-brief.report.json', 'en-media.report.json'];
...
for (const file of files) {
  ...
  for (const id of audits) {
    ...
    console.log(id + ':');
    if (audit.numericValue !== undefined) console.log('  numericValue:', audit.numericValue);
    if (audit.score !== undefined) console.log('  score:', audit.score);
    if (audit.details?.items?.length) {
      console.log('  details.items:', JSON.stringify(audit.details.items, null, 2).split('\n').map(l => '  ' + l).join('\n'));
    } else if (audit.details?.items) {
      console.log('  details.items:', audit.details.items);
    }
    if (audit.details?.debugData) {
      console.log('  debugData:', JSON.stringify(audit.details.debugData, null, 2).slice(0, 800));
    }
    if (id === 'largest-contentful-paint-element' && audit.details?.items?.[0]) {
